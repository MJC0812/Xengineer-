class VoiceDrawingApp {
    ...
    tryExecuteCommand(command, commands) {
        ...
        if (commands.dialectMandarin && commands.dialectMandarin.some(cmd => command.includes(cmd.toLowerCase()))) {
            this.changeDialect('mandarin');
            return true;
        }
        
        if (commands.dialectCantonese && commands.dialectCantonese.some(cmd => command.includes(cmd.toLowerCase()))) {
            this.changeDialect('cantonese');
            return true;
        }
        
        if (commands.dialectSichuan && commands.dialectSichuan.some(cmd => command.includes(cmd.toLowerCase()))) {
            this.changeDialect('sichuan');
            return true;
        }
        
        if (commands.dialectShanghai && commands.dialectShanghai.some(cmd => command.includes(cmd.toLowerCase()))) {
            this.changeDialect('shanghai');
            return true;
        }
        
        if (commands.dialectMinnan && commands.dialectMinnan.some(cmd => command.includes(cmd.toLowerCase()))) {
            this.changeDialect('minnan');
            return true;
        }
        
        if (commands.dialectNortheast && commands.dialectNortheast.some(cmd => command.includes(cmd.toLowerCase()))) {
            this.changeDialect('northeast');
            return true;
        }
        
        if (commands.dialectWu && commands.dialectWu.some(cmd => command.includes(cmd.toLowerCase()))) {
            this.changeDialect('wu');
            return true;
        }
        
        if (commands.dialectHakka && commands.dialectHakka.some(cmd => command.includes(cmd.toLowerCase()))) {
            this.changeDialect('hakka');
            return true;
        }
